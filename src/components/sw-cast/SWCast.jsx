...
function SWCast() {
    ...
    const castList = cast.map((actor, index) =>{
        ...
        return(
            ...
            <SWCard key={index} content={actor.name} date={actor.character} src={`https://image.tmdb.org/t/p/w185/${actorimg}`} button={false} isSW={true}></SWCard>
  ...
  return (
    <>
        ...
        {selectedFilm && <SWCard key={filmid} content={selectedFilm.title} date={selectedFilm.release_date.slice(0, 4)} button={false} isSW={true} src={selectedFilm.poster_path ? `https://image.tmdb.org/t/p/w500/${selectedFilm.poster_path}` : "src/assets/star-wars-2-cropped.svg"}></SWCard>}
        </div>
